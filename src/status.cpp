...
static uint8_t sFxDur;

#define LED_ON  (CONFIG_STATUS_LED_POL == LOW ? LOW : HIGH)
#define LED_OFF (CONFIG_STATUS_LED_POL == LOW ? HIGH : LOW)

#define FX_ON  (CONFIG_STATUS_FX_POL == LOW ? LOW : HIGH)
#define FX_OFF (CONFIG_STATUS_FX_POL == LOW ? HIGH : LOW)
...
static void sStatusLedTimerFunc(void)
{
    ...
    if (sFxDur > 0)
    {
        ...
        if (sFxDur == 0)
        {
            digitalWrite(CONFIG_STATUS_FX_PIN, FX_OFF);;
        }
    }
}
...
{
    DEBUG("status: fx");
    digitalWrite(CONFIG_STATUS_FX_PIN, FX_ON);
    sFxDur = 20;
}
...
{
    if ( statusTonePlaying() || (cfgGetNoise() == CFG_NOISE_NONE) )
    {
        return;
    }
...
void statusInit(void)
{
    DEBUG("status: init (ledPin=" STRINGIFY(CONFIG_STATUS_LED_PIN) ", ledPol=" STRINGIFY(CONFIG_STATUS_LED_POL)
        ", fxPin=" STRINGIFY(CONFIG_STATUS_FX_PIN) ", fxPol=" STRINGIFY(CONFIG_STATUS_FX_POL)
        ", tonePin=" STRINGIFY(CONFIG_STATUS_TONE_PIN) ")");

    pinMode(CONFIG_STATUS_LED_PIN, OUTPUT);
    digitalWrite(CONFIG_STATUS_LED_PIN, LED_OFF);

    pinMode(CONFIG_STATUS_FX_PIN, OUTPUT);
    digitalWrite(CONFIG_STATUS_FX_PIN, FX_OFF);
